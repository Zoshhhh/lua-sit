...
-- Création de threads et gestion des entrées de contrôle
CreateThread(function()
    while true do
        Wait(0)
        if IsControlPressed(0, 19) then  -- 19 pour 'Left Alt'
            local playerPed = PlayerPedId()
            local entity = GetEntityPlayerIsFreeAimingAt(playerPed) -- Obtenez l'entité que le joueur vise
            if entity and IsEntityAnObject(entity) then
                local model = GetEntityModel(entity)
                if Config.Objects[model] then
                    -- Vérifier l'état des ressources et ajouter des interactions contextuelles
...
-- Configuration des textes et des modèles interactifs
CreateThread(function()
...
-- Fonction pour vérifier si le siège est libre
